...
export function NotificationBell({
  ...
  getItemUrl: (candidateJobId: string) => string;
}) {
  ...
  function handleItemClick(candidateJobId: string) {
    setOpen(false);
    router.push(getItemUrl(candidateJobId));
  }
        ...
        <div className="absolute right-0 top-10 z-50 w-80 overflow-hidden rounded-lg border bg-background shadow-xl">
          ...
          {items.length === 0 ? (
            ...
            <div className="max-h-[360px] divide-y overflow-y-auto">
              {items.map((item) => {
                const cfg = TYPE_CONFIG[item.type] ?? TYPE_CONFIG.COMMENT_ADDED;
                const Icon = cfg.icon;
                return (
                  <button
                    key={item.id}
